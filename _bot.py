...
from pytsite import cache, reg, logger, lang, util
from . import _api, types, error
from .reply_markup import ReplyMarkup

_cache_pool = cache.create_pool('telegram.bot_state')
...
class Bot:
    def __init__(self, token: str):
        ...
        self._id = util.md5_hex_digest(str(self.__class__))
        self._state_ttl = reg.get('telegram.bot_state_ttl', 86400)
        self._token = token
        self._me = None
        self._sender = None  # type: types.User
        self._chat = None  # type: types.Chat
    ...
    def _process_private_message(self, msg: types.Message):
    ...
    def _process_callback_query(self, query: types.CallbackQuery):
        # Try to restore current command from state
        if self.command_name:
    ...
    def call_command(self, name: str, msg: Union[types.Message, types.CallbackQuery], step: int = None):
    ...
    def handle_private_message(self, msg: Union[types.Message, types.CallbackQuery]):
        """Hook
        """
        logger.debug('{}: Private message received: {}'.format(self.__class__, msg))

    def handle_command(self, name: str, msg: Union[types.Message, types.CallbackQuery]):
        """Hook
        """
        logger.debug('{}: Command received: {}'.format(self.__class__, msg))

        raise error.CommandExecutionError(lang.t('telegram@unknown_command', {'command': name}))

    def handle_channel_post(self, msg: types.Message):
        """Hook
        """
        logger.debug('{}: Channel post received: {}'.format(self.__class__, msg))

    def handle_inline_query(self, query: types.InlineQuery):
        """Hook
        """
        logger.debug('{}: Inline query received: {}'.format(self.__class__, query))

    def handle_chosen_inline_result(self, result: types.ChosenInlineResult):
        """Hook
        """
        logger.debug('{}: Chosen inline result received: {}'.format(self.__class__, result))

    def handle_callback_query(self, query: types.CallbackQuery):
        """Hook
        """
        logger.debug('{}: Callback query received: {}'.format(self.__class__, query))

    def handle_shipping_query(self, query: types.ShippingQuery):
        """Hook
        """
        logger.debug('{}: Shipping query received: {}'.format(self.__class__, query))

    def handle_pre_checkout_query(self, query: types.PreCheckoutQuery):
        """Hook
        """
        logger.debug('{}: Pre checkout query received: {}'.format(self.__class__, query))
    ...
    def get_me(self) -> types.User:
        ...
        self._me = types.User(self._request('getMe'))

        return self._me

    def send_message(self, text: str, chat_id: Union[int, str] = None, parse_mode: str = 'HTML',
                     disable_web_page_preview: bool = False, disable_notification: bool = False,
                     reply_to_message_id: int = None, reply_markup: ReplyMarkup = None) -> types.Message:
        """Send text message
        ...
        msg = types.Message(self._request('sendMessage', {
            ...
            'reply_markup': json.dumps(reply_markup.as_jsonable()) if reply_markup else '',
        }))
    ...
    def edit_message_text(self, text: str, chat_id: Union[int, str] = None, message_id: int = None,
                          inline_message_id: str = None, parse_mode: str = 'HTML',
                          disable_web_page_preview: bool = False,
                          reply_markup: ReplyMarkup = None) -> types.Message:
        ...
        return types.Message(self._request('editMessageText', {
            'text': text,
            'chat_id': chat_id,
    ...
    def edit_message_caption(self, caption: str = None, chat_id: Union[int, str] = None, message_id: int = None,
                             inline_message_id: str = None, reply_markup: ReplyMarkup = None):
    ...
    def send_photo(self, photo_file_id: str, chat_id: Union[int, str] = None, caption: str = None,
        ...
        """
        msg = types.Message(self._request('sendPhoto', {
            'chat_id': chat_id or self.chat.id,
            'photo': photo_file_id,
            'caption': caption,
            'disable_notification': disable_notification,
    ...
    @staticmethod
    def _sanitize_chat_id(chat_id: Union[int, str]) -> Union[int, str]:
    ...
    def get_chat(self, chat_id: Union[int, str]) -> types.Chat:
        ...
        try:
            return types.Chat(self._request('getChat', {
                'chat_id': chat_id,
            }))

        except error.ApiRequestError as e:
            if 'chat not found' in str(e) or 'bot was kicked':
                raise error.ChatNotFound(chat_id)
            else:
                raise e

    def get_chat_administrators(self, chat_id: Union[int, str]) -> types.ChatMemberArray:
        ...
        try:
            return types.ChatMemberArray(self._request('getChatAdministrators', {
                'chat_id': chat_id,
            }))
    ...
    def get_chat_member(self, chat_id: Union[int, str], user_id: int) -> types.ChatMember:
        ...
        try:
            return types.ChatMember(self._request('getChatMember', {
                'chat_id': chat_id,
                'user_id': user_id,
            }))
    ...
    def can_post_messages(self, chat_id: Union[str, int]) -> bool:
    ...
    def get_file_url(self, file: types.File) -> str:
